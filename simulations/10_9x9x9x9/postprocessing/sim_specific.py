...
def plotsim(simids, fname, stitle):
    ...
    o23 = o23.drop('doy', 1)
    o22.iloc[:, 0].plot(color='lightgray', ax=a5, ylim=[0, 5000], legend=False)
    o22.iloc[:, 1:].plot(ax=a5, linewidth=lw, legend=False)
    a5s.plot([0, 5000], [0, 5000], color='lightgray', linewidth=lw)
    for ci in range(1, o22s.shape[1]):
        a5s.plot(o22s.iloc[:, 0], o22s.iloc[:, ci])
    a5s.set_xlim([0, 5000])
    a5s.set_ylim([0, 5000])
    o23.iloc[:, 0].plot(color='lightgray', ax=a6, ylim=[0, 5000], legend=False)
    o23.iloc[:, 1:].plot(ax=a6, linewidth=lw, legend=False)
    a6s.plot([0, 5000], [0, 5000], color='lightgray', linewidth=lw)
